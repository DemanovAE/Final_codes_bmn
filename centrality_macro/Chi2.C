void Chi2(TString InFileName, const string out_name = "fit_Chi2_f_k.root")
{
  TFile *file = new TFile(InFileName);
  TTree *tree = (TTree *)file->Get("test_tree");
  TTree *delta = new TTree("delta", "delta");

  TFile *ofile = new TFile(out_name.c_str(),"RECREATE");
  TH2D *histo = new TH2D("h2_fk_chi2",";f;k",21,0.,1.05,21,0.,1.05);
  ...
  for (Int_t i = 0; i < n; i++)
  {
    ...
    if ( chi2 < chi2_min)
    //if ( abs(chi2 - 1.0) < abs(chi2_min - 1.0))
    {
      chi2_min = chi2;
  ...
  std::cout << " f = " << f_min << " mu = " << mu_min << " k = " << k_min << " p = " << p_min << " sigma = " << sigma << " chi2 = " << chi2_min << " chi2_error = " << chi2_error << std::endl;
  ...
  //std::cout << " f = " << f_min << " mu = " << mu_min << " k = " << k_min << " p = " << p_min << " sigma = " << sigma << " chi2 = " << chi2_min << " chi2_error = " << chi2_min_error << std::endl;
