...
void CentralityClasses(Int_t CentralityClasses, string InFileGlauberQA, string InFileHistoCut, string OutPath, string OutFile){
	
	bool drawC=false;

	TFile *f1 = new TFile(InFileHistoCut.c_str()); // HistoCutResult.root
	TFile *f5 = new TFile(InFileGlauberQA.c_str());  //glauber_qa.root
	TTree *Borders=(TTree*)f1->Get("Borders");
	
	TFile *fOut   = new TFile(OutFile.c_str(), "recreate"); //FINAL.root
	...
	for (int i = 0; i < CentralityClasses; i++) {
		...
		Npart_VS_CentralityHisto[i]->GetXaxis()->SetRangeUser(0,1000);
		Ncoll_VS_CentralityHisto[i]->GetXaxis()->SetRangeUser(0,1000);
	...
	c1->SaveAs(Form("%s/B_VS_Centrality.png",OutPath.c_str()));
	if(drawC==true)c1->SaveAs(Form("%s/B_VS_Centrality.C",OutPath.c_str()));
	...
	c2->SaveAs(Form("%s/Npart_VS_Centrality.png",OutPath.c_str()));
	if(drawC==true)c2->SaveAs(Form("%s/Npart_VS_Centrality.C",OutPath.c_str()));
	...
	c3->SaveAs(Form("%s/Ncoll_VS_Centrality.png",OutPath.c_str()));
	if(drawC==true)c3->SaveAs(Form("%s/Ncoll_VS_Centrality.C",OutPath.c_str()));	
	...
	legend13.Draw("same");
	c13->Write();
	c13->SaveAs(Form("%s/B_VS_Centrality_Result.png",OutPath.c_str()));
	if(drawC==true)c13->SaveAs(Form("%s/B_VS_Centrality_Result.C",OutPath.c_str()));
	...
	legend14.Draw("same");
	c14->Write();
	c14->SaveAs(Form("%s/Npart_VS_Centrality_Result.png",OutPath.c_str()));
	if(drawC==true)c14->SaveAs(Form("%s/Npart_VS_Centrality_Result.C",OutPath.c_str()));
	...
	legend15.Draw("same");
	c15->Write();
	c15->SaveAs(Form("%s/Ncoll_VS_Centrality_Result.png",OutPath.c_str()));
	if(drawC==true)c15->SaveAs(Form("%s/Ncoll_VS_Centrality_Result.C",OutPath.c_str()));
	...
	c16->cd();
	c16->Write();
	c16->SaveAs(Form("%s/B_VS_Centrality.png",OutPath.c_str()));
	if(drawC==true)c16->SaveAs(Form("%s/B_VS_Centrality.C",OutPath.c_str()));
	...
	legend17.Draw("same");
	c17->Write();
	c17->SaveAs(Form("%s/Npart_VS_Centrality.png",OutPath.c_str()));
	if(drawC==true)c17->SaveAs(Form("%s/Npart_VS_Centrality.C",OutPath.c_str()));
	...
	legend18.Draw("same");
	c18->Write();
	c18->SaveAs(Form("%s/Ncoll_VS_Centrality.png",OutPath.c_str()));
	if(drawC==true)c18->SaveAs(Form("%s/Ncoll_VS_Centrality.C",OutPath.c_str()));
	...
	c19->Write();
	c19->SaveAs(Form("%s/Result.png",OutPath.c_str()));
	if(drawC==true)c19->SaveAs(Form("%s/Result.C",OutPath.c_str()));
