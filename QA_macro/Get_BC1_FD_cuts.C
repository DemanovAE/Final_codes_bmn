...
void Get_BC1_FD_cuts(){	
	...
	TFile *file_inFile 	= new TFile("/home/aleksandr/BM@N/qa_root/qa_run8_vf_24.12_plp.root","READ");
	TFile *file_outFile = new TFile("../OUT_24.04_runId.root","RECREATE");	

	// RunByRun
	std::string path = "/home/aleksandr/BM@N/Final_codes_bmn/pict/QA_RunByRun_24.12";
	...
	DrawObjectParH2.push_back({"h2_RunId_fdInt",suf,0,4,2.,{},{},1});

	RunByRunAnalis(file_inFile, DrawObjectParH2[0],"bc1", 1, path, file_outFile);
	RunByRunAnalis(file_inFile, DrawObjectParH2[1],"fd",  1, path, file_outFile);
	RunByRunAnalis(file_inFile, DrawObjectParH2[0],"bc1", 2, path, file_outFile);
	RunByRunAnalis(file_inFile, DrawObjectParH2[1],"fd",  2, path, file_outFile);
